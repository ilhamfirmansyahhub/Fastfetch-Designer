...
# Keep the source file clean while applying a few install-time compatibility fixes.
# The installed app:
#   - does not expose terminal-font controls
#   - uses readable light preview/UI text
#   - asks Fastfetch for its own config search paths
#   - reloads that config when Refresh is pressed
...
# Remove the preview-only font selector if an older source still contains it.
s = re.sub(
...
# High-contrast default colors, matching the clean light text composition.
s = s.replace("'Default': {'keys': '#D8DEE9', 'title': '#8FBCBB', 'output': '#D8DEE9', 'separator': '#81A1C1'},",
              "'Default': {'keys': '#E6E6E6', 'title': '#F0F0F0', 'output': '#D8DEE9', 'separator': '#9AA6AC'},")
s = s.replace("[('keys','Keys','#D8DEE9'), ('title','Title','#8FBCBB'), ('output','Output','#D8DEE9'), ('separator','Separator','#81A1C1')]",
              "[('keys','Keys','#E6E6E6'), ('title','Title','#F0F0F0'), ('output','Output','#D8DEE9'), ('separator','Separator','#9AA6AC')]")

# Make GTK controls/text readable on dark themes.
s = s.replace("        .terminal-text { color: #D8DEE9; }\n",
...
# Replace the fixed config loader with Fastfetch's own search order.
marker = 'def load_config():\n'
start = s.find(marker)
if start != -1:
    end = s.find('\n\ndef ensure_path', start)
    if end == -1:
        end = s.find('\n\ndef set_path', start)
    if end == -1:
        end = -1
...
# Make Refresh reload the active config from disk instead of only re-rendering old state.
...
# Point the Refresh button to the reload method when the original source uses refresh_preview directly.
...
# Remove the older desktop file name, if present.
